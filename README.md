...
B2-2/
├── .github/
│   ├── CODEOWNERS                 # 파일·폴더별 책임 리뷰어 자동 지정 (보너스)
│   ├── ISSUE_TEMPLATE/
...
├── docs/
│   ├── evidence/
│   │   └── git-log.txt            # git log --oneline --graph --all 결과 (제출 증빙)
│   ├── CONTRIBUTING.md            # 브랜치·커밋·Issue·PR·리뷰 규칙, 충돌 대응 흐름
│   ├── conflict-resolution.md     # 충돌 해결 기록 2회 (둘 다 비자명 충돌)
...
| `docs/evidence/`| `git log --oneline --graph --all` 결과 등 제출 증빙.                              |
